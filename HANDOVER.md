...
- WP-D0 はオッズ推移・当日馬体重/増減・調教詳細をすべて時刻付きで `data/pre-race-signals.jsonl` に追記する。土日09/15時のroutineと午後の収集専用stageに収集処理を設定した。発走後に取得した情報は拒否する。相対値はオッズ変化・馬体重/増減のレース内中央値との差、調教最終1Fの同コース中央値との差を保存する。調教公開ページは一部の馬しか表示されないため欠損を保持し、未取得をゼロに置換しない。`npm run report:pre-race-signals` で3系統と同一レースで揃った件数を確認する。B2の再評価は同一レースで100件以上を蓄積してから時系列分割で実施し、D2/D3/D4は引き続き保留。
- D0の `readyForB2` は、発走前の同一captureで全出走馬に3系統の相対値があり、出走頭数も確認できたレースが100件以上のときだけtrue。公開調教の一部馬だけに値があるレースは `partialJointRelativeRaces` に数える。2026-09-26時点は対象8件、部分的に3系統が見えるレース7件、全頭で揃うレース0件。既存captureの `fieldComplete=null` も全頭確認済みとはみなさない。
- 2026-09-26 の土曜09時 Actions は11:23 JSTに開始された。D0 の午後収集は12:17、13:17/47、14:17、15:47、16:17 JSTの収集専用 stage を追加して遅延・欠落に備える。この stage は routine・結果精算・投稿を実行しない。GitHub Actions の予定時刻どおりの実行は保証されないため、`npm run report:pre-race-signals` で取得実績を継続確認する。
