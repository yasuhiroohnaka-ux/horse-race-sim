...
function formatTrendAdjustment(value?: number): string {
...
function formatPreviousRaceEvidence(horse: Horse): string | null {
  const previousRaceName = horse.previousRaceDisplayName ?? horse.previousRaceName ?? horse.lastRaceName;
  if (!previousRaceName) return null;
...
function hasTrendHint(row: ReturnType<typeof buildRaceAnalysisRows>[number], pattern: string): boolean {
  return row.matchedTrendHints?.some((hint) => hint.id.includes(pattern)) ?? false;
}
...
export function SimulationResults({
  ...
  const adjustedTopRows = hasTrendRows
    ? [...rows].filter((row) => row.adjustedRank !== undefined).sort((a, b) => (a.adjustedRank ?? 999) - (b.adjustedRank ?? 999)).slice(0, 3)
    : [];
  const multiTrendRows = hasTrendRows
    ? [...rows]
      .filter((row) => (row.matchedTrendHints?.length ?? 0) >= 2)
      .sort((a, b) => (b.matchedTrendHints?.length ?? 0) - (a.matchedTrendHints?.length ?? 0))
    : [];
  const nztRows = hasTrendRows ? rows.filter((row) => hasTrendHint(row, "nzt")) : [];
  const nztWinnerRiskRows = hasTrendRows ? rows.filter((row) => hasTrendHint(row, "nzt-winner-risk")) : [];
  const satsukiRows = hasTrendRows ? rows.filter((row) => hasTrendHint(row, "satsuki-sho")) : [];
            ...
            <tr className="border-y border-slate-200 bg-slate-50 text-slate-500">
              ...
              {hasTrendRows && (
                <>
                  ...
                  <th className="px-2 py-2 text-left">trend tags</th>
                </>
              )}
              <th className="px-2 py-2 text-right">
                ...
                <tr key={row.horseId} className="border-b border-slate-100 hover:bg-slate-50/70">
                  ...
                  {hasTrendRows && (
                    <>
                      ...
                      <td className="px-2 py-2">
                        {row.matchedTrendHints && row.matchedTrendHints.length > 0 ? (
                          <div className="flex max-w-[260px] flex-wrap gap-1">
                            {row.matchedTrendHints.slice(0, 3).map((hint) => (
                              <span
                                key={hint.id}
