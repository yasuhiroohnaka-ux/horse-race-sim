...
import { dataStorageKind, readDataFile, writeDataFile } from "./dataFile.mjs";
...
const VALID_STORE_KINDS = new Set(["auto", "file", "vercel_blob", "cloudflare_r2"]);
...
function resolveStoreKind() {
  if (dataStorageKind === "cloudflare_r2") return "cloudflare_r2";
...
function createCloudflareR2Adapter() {
  return {
    ...
    async read() {
      try {
        const raw = await readDataFile(RUNNING_STYLE_OVERRIDES_PATH, "utf8");
    ...
    async write(data) {
      await writeDataFile(RUNNING_STYLE_OVERRIDES_PATH, JSON.stringify(data, null, 2) + "\n", "utf8");
    },
  };
}
...
export function getRunningStyleOverrideStorageAdapter() {
  ...
  cachedAdapter = kind === "cloudflare_r2" ? createCloudflareR2Adapter()
    : kind === "vercel_blob" ? createVercelBlobAdapter() : createFileAdapter();
