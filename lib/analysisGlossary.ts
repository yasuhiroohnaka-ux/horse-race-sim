import { MONTE_CARLO_RUNS_LABEL } from "./simulationConfig";

export const SUMMARY_CARD_DESCRIPTIONS = {
  strongest:
    `${MONTE_CARLO_RUNS_LABEL}試走で最も勝率が高かった馬です。能力、適性、馬場、風、展開の条件をまとめて反映した総合1位です。`,
  marketWatch:
    "一般市場の上位人気の中から、試走が最も弱気だった馬です。市場先行、能力漏れ、条件替わりのどれで人気しているかを見極める対象です。",
  valueHorse:
    "試走勝率が一般市場の想定より高く、さらにガチ勢市場が先に反応していれば加点する回収妙味候補です。",
  disagreement:
    "一般市場とガチ勢市場の見立てが最も割れている馬です。市場の評価分岐点として使います。",
} as const;

export const TABLE_HEADER_DESCRIPTIONS = {
  abilityScore:
    "能力4値に騎手、厩舎、追切、近走を加え、当日の馬場と展開適性も織り込んだシナリオ能力指数です。",
  simWinRate: `${MONTE_CARLO_RUNS_LABEL}試走での1着率です。連対率ではなく勝ち切る確率だけを見ています。`,
  fairOdds: "試走勝率から逆算した理論オッズです。100÷試走勝率で計算します。",
  officialMarket:
    "実オッズから逆算した一般市場の勝率イメージです。人気順と市場期待の基準になります。",
  expertMarket:
    "俺プロ本命人数、予想人数、X話題量から組んだガチ勢市場の勝率イメージです。",
  officialGap:
    "試走勝率から一般市場想定を引いた差です。プラスほど市場より強く、マイナスほど人気先行です。",
  expertGap:
    "試走勝率からガチ勢市場想定を引いた差です。プラスほどガチ勢より強く、マイナスほどガチ勢先行です。",
  marketExpertGap:
    "ガチ勢市場想定から一般市場想定を引いた差です。プラスはガチ勢が強気、マイナスは一般市場が強気です。",
  buzzShare:
    "predictionCountベースの一般支持率です。広く買われているかどうかを見る補助指標です。",
  signal:
    "試走と一般市場、ガチ勢市場のズレを短いラベルで分類したものです。市場注目はさらに内訳を出します。",
  calWinProb:
    "確定済みレース実績でロジスティック再校正した勝率です。単複エンジンのスコア由来で試走勝率とは別系統。分類ゲートと期待値判断はこちらを使います。",
  calPlaceProb:
    "実績再校正した3着内率です。複勝の期待値と分類ゲート (place/skip) の基準になります。",
  marketGap:
    "一般市場の想定勝率と校正勝率の差です。一致帯 (±5pt) が最も安定。過熱大 (市場が15pt以上高い) は単勝の実績回収率が大きく崩れる帯です。過小 (校正側が高い) は妙味に見えて実績単ROI 79%なので強気シグナルではありません。",
} as const;

export const SIGNAL_LABEL_DESCRIPTIONS = {
  市場注目:
    "試走勝率が一般市場想定を8pt以上下回り、かつガチ勢市場が一般市場以上に強気ではないゾーンです。内訳は市場先行、能力漏れ注目、条件替わり注目に分かれます。",
  市場先行:
    "一般市場の人気が先に走っている型です。ガチ勢の追随や条件面の裏付けがまだ弱いケースです。",
  能力漏れ注目:
    "試走は弱気でも、能力順位やガチ勢市場が下支えしている型です。モデルが拾い切れていない余地を示します。",
  条件替わり注目:
    "距離短縮・延長、馬場悪化、展開変化、コース替わりなど当日の条件で浮上余地がある型です。",
  プロ先行妙味:
    "試走勝率が一般市場想定を8pt以上上回り、さらにガチ勢市場が一般市場より2pt以上先行している馬です。",
  市場盲点:
    "試走勝率が一般市場想定を8pt以上上回る馬です。一般市場がまだ十分に評価していない可能性があります。",
  ガチ勢過熱:
    "試走勝率がガチ勢市場想定を6pt以上下回り、なおかつガチ勢市場が一般市場より強気な馬です。",
  ガチ勢先行:
    "ガチ勢市場の想定が一般市場より6pt以上強気な馬です。濃い支持が先行しています。",
  一般人気先行:
    "一般市場の想定がガチ勢市場より6pt以上強気な馬です。広い人気ほど先行しています。",
  やや妙味:
    "試走勝率が一般市場想定を3pt以上上回る馬です。強い買い材料までは届かないが、やや割安です。",
  やや過熱:
    "試走勝率が一般市場想定を3pt以上下回る馬です。大きな危険評価ではないが、少し人気先行です。",
  概ね妥当:
    "試走と市場評価のズレが小さく、今の人気水準がおおむね妥当な馬です。",
} as const;

export function getSignalDescription(label: string, signalReason?: string | null): string {
  const base =
    SIGNAL_LABEL_DESCRIPTIONS[label as keyof typeof SIGNAL_LABEL_DESCRIPTIONS] ??
    "試走と市場評価のズレから見た中間評価です。";
  return signalReason ? `${base} ${signalReason}` : base;
}
