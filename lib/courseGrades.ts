...
export const COURSE_GRADE_VALUES = ["G1", "G2", "G3", "L", "OP", "OTHER"] as const;
...
export function getCourseGrade(course: CourseLike): CourseGrade {
...
export function countCoursesByGrade<T extends CourseLike>(courses: readonly T[]): Record<CourseGrade, number> {
  ...
  for (const course of courses) {
    counts[getCourseGrade(course)] += 1;
  }
