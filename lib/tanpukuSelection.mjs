...
export function classifyHonmeiPick(entry) {
  ...
  if (marketGapLabel === "fair_priced") {
    cautionReasons.push("市場と校正が一致する適正評価帯 (実績単ROI 88-108%)");
  }
  ...
  if (odds >= CLASSIFY.SKIP_ODDS_MIN) {
    return {
      classification: "skip",
      confidence: round3(clamp(0.65 + cautionPenalty, 0.1, 1.0)),
      reason: `本命が${odds.toFixed(1)}倍の混戦。この帯の単的中は実績15%・単ROI66%で単勝は見送り${cautionSuffix}`,
    };
  }
  ...
  if (calWin >= CLASSIFY.WIN_PROB_MIN && calTanRoi >= CLASSIFY.WIN_TAN_ROI_MIN) {
    return {
      ...
      reason: `単勝勝負(本命級): 校正勝率${Math.round(calWin * 100)}%・校正単ROI${Math.round(calTanRoi)}% (実績帯: 単的中44%・単ROI108%)${cautionSuffix}`,
    };
  }
  ...
  if (calWin >= CLASSIFY.WIN_CORE_PROB_MIN && odds < CLASSIFY.WIN_CORE_ODDS_MAX) {
    return {
      ...
      reason: `単勝勝負(堅軸): 校正勝率${Math.round(calWin * 100)}%・${odds.toFixed(1)}倍 (実績帯: 単的中47%・単ROI98%)${cautionSuffix}`,
    };
  }
...
export function pickTanpukuPair(race, includeBodyWeight = false, applyDraw = true) {
  ...
  const wideRecommendation = (() => {
    ...
    return {
      ...
      reason: "live OOS基準未達（確定96R中place分類69件、ワイドROI 60.0%；直近50R内place35件、66.9%）のため推奨停止。ペア決済のシャドー監視は継続",
    };
  })();
