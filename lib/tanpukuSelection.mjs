export const TANPUKU_SCORING_VERSION = "tanpuku-place-v2.2";
...
export function pickTanpukuPair(race, includeBodyWeight = false, applyDraw = true) {
  ...
  const placeSorted = [...scored].sort((a, b) => b.placeScore - a.placeScore || b.placeProb - a.placeProb);

  const winPickBase = placeSorted[0];

  // Quality gate: filter value candidates with minimum viability
  const valueCandidates = scored.filter(
    (entry) =>
      entry.horse.id !== winPickBase.horse.id &&
      entry.placeProb >= VALUE_MIN_PLACE_PROB &&
      entry.top3Stability >= VALUE_MIN_TOP3_STABILITY &&
      entry.overbetLabel !== "overbet_high"
  );
  const valueSorted = [...valueCandidates].sort(
    (a, b) => b.valueScore - a.valueScore || b.placeReturnEdge - a.placeReturnEdge
  );
  ...
  const valuePick = valuePickBase
    ? {
        ...
        selectionReason: buildSelectionReason(
          "value",
          valuePickBase,
          valuePickBase.valueScore - (valueRunnerUp?.valueScore ?? 0)
        ),
      }
    : null;
  ...
  return {
    ...
    valuePick: valuePick
      ? {
          ...valuePick,
          placeScore: round3(valuePick.placeScore),
          valueScore: round3(valuePick.valueScore),
          placeProb: round3(valuePick.placeProb),
          winProb: round3(valuePick.winProb),
          placeOdds: round1(valuePick.placeOdds),
          tanRoi: round1(valuePick.tanRoi),
          fukuRoi: round1(valuePick.fukuRoi),
          top3Stability: round3(valuePick.top3Stability),
          marketSupport: round3(valuePick.marketSupport),
          overbetRisk: round3(valuePick.overbetRisk),
          placeReturnEdge: round3(valuePick.placeReturnEdge),
          longshotBonus: round3(valuePick.longshotBonus),
        }
      : null,
  };
}
