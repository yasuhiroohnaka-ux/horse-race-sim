...
export const TANPUKU_SCORING_VERSION = "tanpuku-win-v3.1";
export { CALIBRATION_VERSION };

// ---------------------------------------------------------------------------
// Classification constants (v3.1: 単勝主軸ゲート)
// ---------------------------------------------------------------------------
// v2.4 で導入した校正値ベースのゲートを踏襲しつつ、v3.1 では判定層を
// 「単勝を買ってよいか」を主軸に組み直した。
//
// 検証方法 (scripts/backtest-selection.mjs):
// - 選定に渡すオッズは review-records の事前スナップショット値
//   (アーカイブの horses[].realOdds は確定オッズが混入しておりリークになる)
// - 採点は公式単勝払戻。事前オッズ×100 を払戻とみなすと穴馬帯で約2倍過大になる
// - 閾値は前半166件だけで決め、後半167件で検証した
//
// 分割検証で「両半期とも単調」だったのは calWinProb と odds の2つだけだった。
// calTanRoi は前半と後半で符号が反転し (前半 15/95/99/109 → 後半 122/78/83/53)、
// fieldSize も反転する (前半 62/53/106/165 → 後半 106/66/79/68)。
// このため v3.1 のゲートは calWinProb と odds のみを判定に使う。
//
// 選定順位 (placeScore) は変更していない。本命の選び方を
// placeScore / 能力スコア / 市場1番人気 の3方式で比較したが、全333件で
// 的中 31.2% / 30.3% / 30.6%、単ROI 82.0% / 81.7% / 80.2% とほぼ同着であり、
// 選定順位の入れ替えに再現性のある利得はなかった。価値は判定層にある。

const CLASSIFY = {
  // --- skip: 単勝が期待できないセグメント ---
  // 本命が4.0倍以上に沈むレースは市場が割れており、単的中は前半17.0%/後半10.0%、
  // 単ROI は前半83.4%/後半42.8%。両半期で的中率が低いので単勝は買わない。
  SKIP_ODDS_MIN: 4.0,
  SKIP_PLACE_PROB_MAX: 0.52,  // 校正複勝率がこれ未満かつ
  SKIP_TAN_ROI_MAX: 85,       // 校正単ROI がこれ未満なら見送り

  // --- win: 単勝勝負の2段ゲート ---
  // T1 本命級: 従来の v2.4 ゲート。全333件で単的中 44.2% / 単ROI 108.1%
  //            (前半 117.1% / 後半 99.5%) と再現性がある。
  WIN_PROB_MIN: 0.35,         // 校正勝率の下限
  WIN_TAN_ROI_MIN: 95,        // 校正単ROI (calWinProb * odds * 100) の下限
  // T2 堅軸: 校正勝率が立ち、かつ市場も本命視している帯。
  //          全333件で単的中 46.8% / 単ROI 98.2% (前半 102.4% / 後半 94.9%)。
  //          閾値まわりの ROI 曲面は 0.24〜0.34 で 94.6〜98.6% とほぼ平坦なので、
  //          この 0.30 という値自体は結論を左右しない。
  WIN_CORE_PROB_MIN: 0.30,
  WIN_CORE_ODDS_MAX: 4.0,
  ...
  // 少頭数 (<=9頭) は v2.4 で hard skip だったが、公式払戻で採点し直すと
  // 前半 単的中8.3%/単ROI14.2% → 後半 57.1%/102.9% と反転し再現しなかった。
  // 根拠だった n=18 は小さすぎたため、hard skip をやめて caution に降格する。
  SMALL_FIELD_MAX: 9,
  SMALL_FIELD_PENALTY: 0.10,
};

export const D1_SHADOW_POLICY_ID = "overbet-high-t2-to-place-v1";

/**
 * Classify a honmei pick as win / place / skip (v3.1: 単勝主軸ゲート).
 *
 * The honmei is still the placeScore composite leader (selection order unchanged).
 ...
 */
export function classifyHonmeiPick(entry, { d1Shadow = false } = {}) {
  if (!entry) {
  ...
  // fair_priced は説明のみ追加し confidence は変えない (採用基準未達のため。
  // 根拠: 分割検証 前半 単ROI108% / 後半88%、方向は再現するが絶対値が基準95%未満)
  const marketGapLabel =
    entry.marketGapLabel ??
    (odds > 0 ? buildMarketGapLabel(clamp(1 / odds, 0.02, 0.7), calWin) : null);
  if (marketGapLabel === "fair_priced") {
    cautionReasons.push("市場と校正が一致する適正評価帯");
  }
  if (fieldSize > 0 && fieldSize <= CLASSIFY.SMALL_FIELD_MAX) {
    cautionPenalty += CLASSIFY.SMALL_FIELD_PENALTY;
    cautionReasons.push(`少頭数${fieldSize}頭`);
  }
  ...
  if (calPlace < CLASSIFY.SKIP_PLACE_PROB_MAX && calTanRoi < CLASSIFY.SKIP_TAN_ROI_MAX) {
    return {
      ...
      reason: `校正複勝率${Math.round(calPlace * 100)}%・校正単ROI${Math.round(calTanRoi)}%とも基準未満。見送り${cautionSuffix}`,
    };
  }

  // --- skip: 本命が4.0倍以上に沈む混戦。単的中が両半期で10-17%しかない ---
  if (odds >= CLASSIFY.SKIP_ODDS_MIN) {
    return {
      classification: "skip",
      confidence: round3(clamp(0.65 + cautionPenalty, 0.1, 1.0)),
      reason: `本命が${odds.toFixed(1)}倍の混戦。単勝勝負の事前オッズ条件を満たさないため見送り${cautionSuffix}`,
    };
  }

  // --- win T1: 本命級 (校正勝率と校正単ROI の両方が立つ) ---
  if (calWin >= CLASSIFY.WIN_PROB_MIN && calTanRoi >= CLASSIFY.WIN_TAN_ROI_MIN) {
    return {
      classification: "win",
      confidence: round3(clamp(0.7 - cautionPenalty, 0.1, 1.0)),
      reason: `単勝勝負(本命級): 校正勝率${Math.round(calWin * 100)}%・校正単ROI${Math.round(calTanRoi)}%${cautionSuffix}`,
    };
  }

  // --- win T2: 堅軸 (校正勝率が立ち、市場も本命視している) ---
  if (calWin >= CLASSIFY.WIN_CORE_PROB_MIN && odds < CLASSIFY.WIN_CORE_ODDS_MAX) {
    if (d1Shadow && overbetLabel === "overbet_high") {
      return {
        classification: "place",
        confidence: round3(clamp(0.6 - cautionPenalty, 0.1, 1.0)),
        reason: `過熱リスク大のため単勝勝負を抑えへ変更: 校正勝率${Math.round(calWin * 100)}%・${odds.toFixed(1)}倍${cautionSuffix}`,
      };
    }
    return {
      classification: "win",
      confidence: round3(clamp(0.6 - cautionPenalty, 0.1, 1.0)),
      reason: `単勝勝負(堅軸): 校正勝率${Math.round(calWin * 100)}%・${odds.toFixed(1)}倍${cautionSuffix}`,
    };
  }
  ...
  if (calPlace >= CLASSIFY.PLACE_PROB_MIN) {
    return {
      classification: "place",
      confidence: round3(clamp(0.6 - cautionPenalty, 0.1, 1.0)),
      reason: `3着内安定寄り: 校正複勝率${Math.round(calPlace * 100)}%${cautionSuffix}`,
    };
  }
  ...
  return {
    ...
    reason: `中間帯: 校正複勝率${Math.round(calPlace * 100)}% / 校正勝率${Math.round(calWin * 100)}%。抑えとしては根拠薄め${cautionSuffix}`,
  };
}
...
// v2.5.1: 市場ギャップの4値ラベル (overbetLabel の上位互換、分析・表示用)。
// fair_priced (±5pt一致) は実績で最良帯 (単ROI 前半108%/後半88%) だが、
// 後半の絶対値が採用基準95%に届かないため confidence には反映しない。
// 確定+100件で再判定 (vault: engine-improvement-roadmap フェーズ1-1)。
function buildMarketGapLabel(officialImplied, calWinProb) {
  const gap = officialImplied - calWinProb;
  if (gap >= 0.15) return "overbet_high";
  if (gap >= 0.05) return "overbet_moderate";
  if (gap > -0.05) return "fair_priced";
  return "underbet";
}
...
function buildSelectionReason(kind, entry, gapToRunnerUp) {
  ...
  if (kind === "win") {
    return `総合評価1位。${strongest.join("・")}を重視。${gapText}`.trim();
  }
...
function buildScoredEntry(horse, race, includeBodyWeight, drawMap) {
  ...
  return {
    ...
    marketGapLabel: buildMarketGapLabel(officialImplied, calWinProb),
  };
}
...
export function pickTanpukuPair(race, includeBodyWeight = false, applyDraw = true) {
  ...
  const shadowD1 = {
    policyId: D1_SHADOW_POLICY_ID,
    classificationHint: classifyHonmeiPick(winPick, { d1Shadow: true }),
  };

  // place 分類のワイドはペアを記録し続けるが、live OOS の基準未達により推奨を停止する。
  // 確定96R: place 69件のワイドROI 60.0%（直近50R内 place 35件では66.9%）。
  const wideRecommendation = (() => {
    if (classificationHint.classification !== "place") {
      return {
        recommended: false,
        shadowOnly: false,
        reason: `本命分類が${classificationHint.classification}のため対象外 (ワイドはplace分類のみシャドー監視)`,
      };
    }
    if (!opponentPick) {
      return { recommended: false, shadowOnly: false, reason: "相手候補が存在しないためワイド推奨なし" };
    }
    return {
      recommended: false,
      shadowOnly: true,
      ...
      reason: "live OOS基準未達のため推奨停止。ペア決済のシャドー監視は継続",
    };
  })();
