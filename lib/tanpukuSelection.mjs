...
// ---------------------------------------------------------------------------
// Classification constants (tune these to refine win/place/skip boundaries)
// ---------------------------------------------------------------------------
// The honmei candidate is the placeScore composite leader — NOT a pure win pick.
// Classification asks: "does this leader also have strong win credentials,
// or is it primarily place-oriented, or are both too weak to recommend?"

const CLASSIFY = {
  // --- win thresholds (candidate has genuine win-through potential) ---
  WIN_PROB_MIN: 0.18,       // simulated win probability floor for "win" classification
  TAN_ROI_MIN: 80,          // expected tan return (winProb * odds * 100) floor
  SCORE_GAP_MIN: 0.015,     // minimum placeScore gap over runner-up (dominance signal)

  // --- place thresholds (stable top-3 candidate) ---
  PLACE_PROB_MIN: 0.50,     // minimum placeProb for reliable place classification
  TOP3_STABILITY_MIN: 0.30, // minimum top3Stability for place confidence

  // --- caution signals (contribute to skip, not instant-kill) ---
  OVERBET_HIGH_PENALTY: 0.25, // confidence reduction when overbet_high
  OVERBET_MOD_PENALTY: 0.10,  // confidence reduction when overbet_moderate

  // --- skip: "insufficient evidence for either win or place" ---
  // No single condition triggers skip. Skip = failed both win AND place gates.
};

/**
 * Classify a honmei pick as win / place / skip.
 *
 * The honmei is already the placeScore composite leader (stability-weighted top pick).
 * This function asks: does this leader ALSO have win-through strength, or is it
 * primarily place-oriented, or are both dimensions too weak to recommend?
 *
 * @param {object} entry - scored entry from buildScoredEntry (winPick)
...
export function classifyHonmeiPick(entry) {
  ...
  const winProb = Number(entry.winProb ?? 0);
  const placeProb = Number(entry.placeProb ?? 0);
  const top3Stability = Number(entry.top3Stability ?? 0);
  const tanRoi = Number(entry.tanRoi ?? 0);
  const scoreGap = Number(entry.scoreGap ?? 0);
  const overbetLabel = entry.overbetLabel ?? null;

  // --- Evaluate win credentials ---
  const hasWinProb = winProb >= CLASSIFY.WIN_PROB_MIN;
  const hasWinRoi = tanRoi >= CLASSIFY.TAN_ROI_MIN;
  const hasDominance = scoreGap >= CLASSIFY.SCORE_GAP_MIN;
  const winSignals = [hasWinProb, hasWinRoi, hasDominance].filter(Boolean).length;

  // --- Evaluate place credentials ---
  const hasPlaceProb = placeProb >= CLASSIFY.PLACE_PROB_MIN;
  const hasStability = top3Stability >= CLASSIFY.TOP3_STABILITY_MIN;
  const placeSignals = [hasPlaceProb, hasStability].filter(Boolean).length;

  // --- Caution penalty (reduces confidence, does NOT auto-skip) ---
  ...
  // win: strong win credentials (at least 2 of 3 win signals)
  if (winSignals >= 2 && hasWinProb) {
    const base = 0.5 + winSignals * 0.15;
    const confidence = clamp(base - cautionPenalty, 0.1, 1.0);
    const parts = [];
    if (hasWinProb) parts.push(`勝率${Math.round(winProb * 100)}%`);
    if (hasWinRoi) parts.push(`単ROI${Math.round(tanRoi)}%`);
    if (hasDominance) parts.push(`次点差${scoreGap.toFixed(3)}`);
    const cautionSuffix = cautionReasons.length > 0 ? ` (${cautionReasons.join("・")})` : "";
    return {
      classification: "win",
      confidence: round3(confidence),
      reason: `勝ち切り期待が高い: ${parts.join("・")}${cautionSuffix}`,
    };
  }

  // place: strong place credentials (both signals met)
  if (placeSignals >= 2) {
    const base = 0.5 + placeSignals * 0.2;
    const confidence = clamp(base - cautionPenalty, 0.1, 1.0);
    const parts = [];
    if (hasPlaceProb) parts.push(`複勝率${Math.round(placeProb * 100)}%`);
    if (hasStability) parts.push(`3着内安定${Math.round(top3Stability * 100)}%`);
    const cautionSuffix = cautionReasons.length > 0 ? ` (${cautionReasons.join("・")})` : "";
    return {
      classification: "place",
      confidence: round3(confidence),
      reason: `3着内安定寄り: ${parts.join("・")}${cautionSuffix}`,
    };
  }

  // place (relaxed): at least one place signal and some win potential
  if (placeSignals >= 1 && winSignals >= 1) {
    const confidence = clamp(0.4 - cautionPenalty, 0.1, 1.0);
    return {
      classification: "place",
      confidence: round3(confidence),
      reason: `安定寄りだが根拠はやや薄い: 複勝率${Math.round(placeProb * 100)}% / 勝率${Math.round(winProb * 100)}%${cautionReasons.length > 0 ? ` (${cautionReasons.join("・")})` : ""}`,
    };
  }

  // skip: insufficient evidence for either win or place
  const skipParts = [];
  if (!hasWinProb) skipParts.push(`勝率${Math.round(winProb * 100)}%不足`);
  if (!hasPlaceProb) skipParts.push(`複勝率${Math.round(placeProb * 100)}%不足`);
  if (!hasStability) skipParts.push(`安定度${Math.round(top3Stability * 100)}%不足`);
  skipParts.push(...cautionReasons);

  return {
    classification: "skip",
    confidence: round3(clamp(0.3 + cautionPenalty, 0.1, 1.0)),
    reason: `軸には弱く見送り寄り: ${skipParts.join("・")}`,
  };
}
