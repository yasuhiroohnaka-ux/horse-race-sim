...
function stripNamedRaceClassSuffix(label) {
  return normalizeString(label)
    .replace(/[（(](?:(?:[123\uff11\uff12\uff13])\s*\u52dd\u30af\u30e9\u30b9|(?:[123\uff11\uff12\uff13])Win)[)）]\s*$/iu, "")
    .trim();
}

function isGenericRaceLabel(label) {
  const normalized = normalizeString(label);
  if (!normalized) return false;
  return GENERIC_CLASS_PATTERNS.some((pattern) => pattern.test(normalized));
}
...
function isNamedConditionSpecial(label, raceNumber) {
  ...
  const normalized = normalizeString(label);
  const stripped = stripNamedRaceClassSuffix(normalized);
  if (!stripped) return false;
  if (isGenericRaceLabel(stripped)) return false;
  return true;
}
