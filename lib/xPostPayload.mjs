...
export function buildPreRacePostPayload({ day, race, tanpukuPair, simBestHorse }) {
  ...
  if (opponentPick) {
    lines.push(
      `▶相手候補: ${opponentPick.horse.name}(同軸次点 placeScore${opponentPick.placeScore.toFixed(3)} / 本命差${opponentPick.scoreGap.toFixed(3)})`
    );
  }

  if (opponentPick) {
    lines[lines.length - 1] =
      `笆ｶ相手候補: ${opponentPick.horse.name}(安定次点 placeProb${pct(opponentPick.placeProb)}% / top3安定${pct(opponentPick.top3Stability)}% / placeScore${opponentPick.placeScore.toFixed(3)})`;
    lines.push("本線は単複本命＋相手候補、配当妙味はワイド高配当狙いへ分離");
  }
  ...
  if (widePick) {
    lines.push(
      `▶ワイド高配当狙い: ${widePick.horse.name}(${Number(widePick.horse.realOdds).toFixed(1)}倍 複回収${widePick.fukuRoi.toFixed(0)}%)`
    );
    lines.push("本線の相手ではなく、配当妙味を優先した別軸候補");
  }

  if (widePick) {
    lines[lines.length - 1] =
      `笆ｶワイド高配当狙い: ${widePick.horse.name}(${Number(widePick.horse.realOdds).toFixed(1)}倍 / 複回収${widePick.fukuRoi.toFixed(0)}%)`;
    lines.push("相手候補とは分離した、妙味寄りの別軸");
  }
  ...
  if (widePick) {
    lines.splice(
      lines.length - 3,
      3,
      `相手候補とは分離したワイド高配当狙い: ${widePick.horse.name}(${Number(widePick.horse.realOdds).toFixed(1)}倍 / 複回収${widePick.fukuRoi.toFixed(0)}%)`,
      "妙味・valueScore はワイド高配当狙い側で扱う別軸"
    );
  }
  ...
  if (
    ...
    (!opponentPick || marketFav.id !== opponentPick.horse.id) &&
    (!widePick || marketFav.id !== widePick.horse.id)
  ) {
...
export function buildWeeklySummaryPostPayload({ weeklyPerf, recs, weekOf }) {
  ...
  if (valueBets > 0) {
    lines.push(`▶ワイド高配当狙い 複勝率${pct1(valueFukuRate)}%(${valueBets}R)`);
  }
...
export function buildManualPreRacePayload({
  ...
  if (widePick) {
    ...
    lines.push("本線は単複本命＋相手候補、こちらは配当妙味の別軸");
  }
  ...
  if (
    ...
    (!opponentPick || marketFav.horse.id !== opponentPick.horse.id) &&
    (!widePick || marketFav.horse.id !== widePick.horse.id) &&
...
export function buildManualReviewPayload({
  ...
  if (valueRec) {
    ...
    lines.push(`▶ワイド高配当狙い: ${valueRec.horseName} → ${valueMark}`);
  }
  ...
  return {
    ...
    missTags,
    scoringVersion: null,
    text: (missTags.length > 0 ? [...lines, `missTags: ${missTags.join(", ")}`] : lines).join("\n"),
  };
}
