...
export function summarizeSignalReadiness(captures) {
  ...
  for (const capture of captures) {
    ...
    const validSourceTime = (value) => {
      const time = Date.parse(value ?? "");
      return Number.isFinite(time) && time <= at && time < start;
    };
    const horses = sourceHorses.filter((horse) => {
      if ((horse.odds != null || horse.relative?.oddsMoveVsField != null) &&
        !validSourceTime(horse.oddsFetchedAt)) return false;
      if ((horse.bodyWeightKg != null || horse.bodyWeightDiffKg != null ||
        horse.relative?.bodyWeightVsFieldKg != null) &&
        !validSourceTime(horse.bodyWeightFetchedAt)) return false;
      if ((horse.training != null || horse.relative?.trainingFinal1fVsCourseSeconds != null) &&
        !validSourceTime(horse.trainingFetchedAt)) return false;
      return true;
    });
    ...
    const fullFieldValid = capture.fieldComplete === true && horses.length > 0 &&
      horses.length === sourceHorses.length;
    observed.fullOdds ||= fullFieldValid && horses.every((horse) => horse.odds != null);
    observed.fullBodyWeight ||= fullFieldValid && horses.every((horse) =>
      horse.bodyWeightKg != null && horse.bodyWeightDiffKg != null);
    observed.fullTraining ||= fullFieldValid && horses.every((horse) => horse.training != null);
    observed.jointFullRelative ||= fullFieldValid && horses.every((horse) =>
      horse.relative?.oddsMoveVsField != null && horse.relative?.bodyWeightVsFieldKg != null &&
      horse.relative?.bodyWeightDiffVsFieldKg != null &&
      horse.relative?.trainingFinal1fVsCourseSeconds != null);
    byRace.set(String(capture.raceId), observed);
  }
  const count = (key) => [...byRace.values()].filter((race) => race[key]).length;
  const partialJointRelativeRaces = [...byRace.values()].filter((race) =>
    race.oddsMovement && race.bodyWeight && race.trainingFinal1f).length;
  const jointRelativeRaces = count("jointFullRelative");
  return {
    raceCount: byRace.size, minimumRaces: 100, jointRelativeRaces,
    partialJointRelativeRaces,
    relative: {
