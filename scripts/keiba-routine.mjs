...
// 決定ロジックは lib/recommendedBetDecisionCore.mjs の単一実装を使う
// (以前はここに lib/recommendedBetAction.ts と同内容の複製があり乖離リスクがあった)
function resolveRecommendedBetAction(classificationHint) {
  return resolveRecommendedBetActionFromClassificationHint(classificationHint);
}
...
async function handleSundaySettle(stage = "sun_18") {
  ...
  try {
    ...
    console.warn(`校正レポートの実行に失敗しました (継続): ${err.message}`);
  }
}
