...
function resolveRecommendedBetAction(classificationHint) {
...
function buildRecommendedBetDecision(input = {}) {
  const classification = ["win", "place", "skip"].includes(input.classificationHint?.classification)
    ? input.classificationHint.classification
    : null;
  const oddsSource = String(input.oddsSource ?? "").trim().toLowerCase();
  const overbetLabel = String(input.overbetLabel ?? "").trim().toLowerCase();
  const scoreGap = Number.isFinite(Number(input.scoreGap)) ? Number(input.scoreGap) : null;
  const placeProb = Number.isFinite(Number(input.placeProb)) ? Number(input.placeProb) : null;
  const top3Stability = Number.isFinite(Number(input.top3Stability)) ? Number(input.top3Stability) : null;
  const fieldSize = Number.isFinite(Number(input.fieldSize)) ? Number(input.fieldSize) : null;
  const reasons = [];
  const riskFlags = [];
  ...
  if (classification === "skip") {
    return { action: "skip", confidence: "medium", reasons: [...new Set([...reasons, "classification_hint_skip"])], riskFlags: [...new Set(riskFlags)], source: "explicit_live_rule" };
  }
  ...
  if (hasTinyLargeFieldRisk || hasWeakStabilityRisk || hasDisagreementRisk) {
    ...
    return { action: "skip", confidence: "low", reasons: [...new Set([...reasons, ...skipReasons])], riskFlags: [...new Set(riskFlags)], source: "explicit_live_rule" };
  }
  ...
  if (missingActionInputs.length > 0) {
    return { action: "unknown", confidence: "unknown", reasons: [...new Set(reasons)], riskFlags: [...new Set([...riskFlags, ...missingActionInputs])], source: "safety_rule" };
  }
  ...
  if (winQualified) {
    return { action: "win", confidence: riskFlags.length ? "low" : "high", reasons: [...new Set([...reasons, "strict_win_gate", "strong_score_gap", "high_place_probability", "stable_top3", "engine_agreement"])], riskFlags: [...new Set(riskFlags)], source: "explicit_live_rule" };
  }
  ...
  const placeQualified =
    ...
    overbetLabel !== "overbet_high";
  if (placeQualified) {
    return { action: "place", confidence: riskFlags.length ? "low" : "medium", reasons: [...new Set([...reasons, "place_safety_gate", "sufficient_score_gap", "place_probability_ok", "top3_stability_ok"])], riskFlags: [...new Set(riskFlags)], source: "explicit_live_rule" };
  }
  ...
  return {
    ...
    confidence: "unknown",
    reasons: [...new Set(reasons)],
    riskFlags: [...new Set([...riskFlags, "explicit_action_gate_not_met"])],
    source: "safety_rule",
  };
}
...
async function handleRecommendation(day, stage) {
  ...
  if (tanpuku) {
    ...
    state.tanpukuRecommendations.push({
      ...
      recommendedBetDecision: winRecommendedBetDecision,
    ...
    if (valuePick) {
      state.tanpukuRecommendations.push({
        ...
        recommendedBetDecision: buildRecommendedBetDecision({
          sourceStatus: "live_pre_race",
          livePreRaceEligible: true,
          classificationHint: null,
          oddsSource: valuePick.horse?.oddsSource ?? null,
          hasSelectionLog: false,
        }),
